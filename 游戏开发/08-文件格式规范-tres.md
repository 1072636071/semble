# .tres 资源文件格式（godot-tres-format）

> Godot 资源文件是纯文本，AI 可直接通过 `write_to_file` 创建材质、环境、渐变等。
...
| `emission` | Color | 发光颜色 |
| `emission_energy_multiplier` | float | 发光强度 |
| `transparency` | int | 0=不透明, 1=Alpha, 2=预乘Alpha |
| `cull_mode` | int | 0=背面剔除, 1=正面, 2=不剔除 |
...
```
红色: Color(1, 0, 0, 1); 绿色: Color(0, 1, 0, 1); 蓝色: Color(0, 0, 1, 1)
