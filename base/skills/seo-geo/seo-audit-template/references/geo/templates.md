...
即贴即用的配置、代码和 schema。将名称和 URL 适配到用户域名。不要捏造字段。
...
放置于 `/.well-known/mcp/server-card.json`：
...
放置于 `/.well-known/agent-card.json`：
...
放置于 `/.well-known/oauth-authorization-server`：
...
放置于 `/.well-known/oauth-protected-resource`：
...
从页面的 Markdown 版本中剥离此。

## Markdown 内容协商（Express 中间件）
...
## GA4 AI Referral Traffic regex（再贴一次）

在 GA4 → Admin → Data display → Channel groups 中，设置 Source matches regex：
...
稳定后提交到 https://hstspreload.org。

## 适配说明

粘贴这些模板时：

1. 将 `example.com` 替换为用户域名。
2. 将占位名（Example Inc.、Brand Name、Author Name）替换为真实值。
3. 将日期更新为当前。
4. 在 https://search.google.com/test/rich-results 和 https://validator.schema.org 验证 JSON-LD。
5. 手动对照 https://llmstxt.org 规范验证 llms.txt 结构。
6. 对 OAuth、MCP 和 A2A 模板，对照来源处的最新版本核实规范版本。
7. 部署后用 `curl` 测试。真实验证胜过假设。
