...
Trendshift GitHub Trending #1 Repository of the Day
...
小红书 OpenCLI（桌面） xiaohongshu-mcp（服务器）▸ xhs-cli xhs-cli 作者已转投 OpenCLI（24K Star）；浏览器登录态零摩擦
LinkedIn linkedin-scraper-mcp Jina Reader MCP 服务，浏览器自动化
...
友情链接
方舟 Agent Plan 模型订阅套餐 — 集成了包含 Doubao-Seed、Doubao-Seedance、Doubao-Seedream 等在内的字节跳动自研 SOTA 级模型，覆盖文本、代码、图像、视频等多模态任务。最新支持 MiniMax-M3、DeepSeek-V4 系列、GLM-5.2、Doubao-Seed-2.0 系列、Kimi-K2.6 等模型，工具不限。超全模态模型与 Harness 升级一步到位，深度支持 Agent 框架与 AI 编程工具。一次订阅，可以为不同任务切换合适的 AI 引擎。
