...
- **一次性抛三个问题** — 逐个引导用户完成三个决策，展示一部分、获取回答、再进入下一个。
- **不探索就假设** — 先读取仓库已有内容（AGENTS.md / CONTEXT.md / docs / .scratch），不假设。
- **CLAUDE.md 与 AGENTS.md 同时存在都建** — 存在一个就编辑它，永远不创建另一个。
- **重复追加 `## Agent skills` 块** — 已存在则就地更新，不追加重复块。

## 异常处理

| 场景 | 处理方式 |
|------|---------|
| CLAUDE.md 存在 | 编辑它，不创建 AGENTS.md |
| 两者都不存在 | 默认创建 `AGENTS.md`（非交互路径下直接创建，不询问） |
| 用户说"全部默认" | 跳过步骤 2-3 的逐一询问，直接套用默认配置进入步骤 4 |
| 本地无 PR | 跳过"PR 作为请求渠道"的追问，直接用本地 markdown tracker |
