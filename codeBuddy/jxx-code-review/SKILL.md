...
- **一次性 review 大量变更** — 抓住整条分支从上到下逐个 review。diff 越大越难抓准，越容易遗漏。聚焦本次变更范围。
- **跨维度合并发现** — 把标准问题和 spec 问题混在一个报告里。两个维度刻意分离（见*为什么两个维度*）。
- **未确认固定点就 diff** — 固定点不可解析或 diff 为空仍在子 agent 内硬跑。在步骤 1 就中止。
- **在提示中塞满 diff 文本** — 用 `/.temp/` 持久化 diff 文件，让子 agent 自行读取，而非传递大量文本。
- **替 spec 子 agent 编造 spec** — spec 缺失时让子 agent 跳过并报告"无可用 spec"，而非臆造。

## 异常处理

| 场景 | 处理方式 |
|------|---------|
| 固定点不可解析（`git rev-parse` 失败） | 在步骤 1 中止，提示用户提供有效引用 |
| diff 为空 | 无变更可审，告知用户无需 review |
| 未找到 spec | spec 子 agent 跳过，最终报告注明"无可用 spec" |
| 仓库无编码标准文档 | 标准轴仅携带异味基线（始终是酌情判断） |
