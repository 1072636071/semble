---
name: jxx-git-guardrails-claude-code
description: 设置 Claude Code hooks（PreToolUse），在执行前拦截并阻止危险的 git 命令（push、reset --hard、clean、branch -D 等）；当用户想防止破坏性 git 操作、添加 git 安全 hooks 或在 Claude Code 中阻止 git push/reset 时使用。
metadata:
  version: 1.0.0
---
...
创建 PreToolUse hook，在 Claude 执行之前拦截并阻止危险的 git 命令。
...
被阻止时，Claude 会看到一条消息，告知其无权执行这些命令。
...
应以退出码 2 退出，并在 stderr 中打印 BLOCKED 消息。
