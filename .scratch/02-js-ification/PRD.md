...
以 Node（`.mjs`）为唯一脚本语言，建立 `base/scripts/` 下的共享工具层与各专题 CLI，把每个技能的"确定性流程"从"模型对话即兴"收敛为"调用脚本"；把现存 Python/bash/PowerShell 脚本跨平台重写为 `.mjs`；把根目录 PS 脚本改为配置驱动的 `install.mjs`/`verify.mjs`。程序按专题拆分为多份 PRD（见下"超出范围"中的专题清单），本 PRD 定义总纲——共享工具层、统一 seam、依赖策略、专题划分与优先序。
...
2. 作为技能维护者，我想要一个共享工具层（`base/scripts/*.mjs`）可被任意技能引用，以便各专题不各写一套重复工具。
...
5. 作为技能维护者，我想要根目录 `install.ps1`/`verify.ps1` 改为配置驱动的 `install.mjs`/`verify.mjs`（源/目标/清单外置 `skills.config.json`），以便消除硬编码路径、可跨平台。
...
- **共享工具层**：新建 `base/scripts/` 下跨技能复用的 `.mjs`——`append-fragment.mjs`（安全追加 md，分隔符/不覆盖）、`next-seq.mjs`（目录扫描递增编号 + slug）、`render-template.mjs`（模板实例化 + 冲突加序号不覆盖）、`report-html.mjs`（结构化数据 → 自包含 HTML + 临时目录 + 时间戳 + 跨平台打开）、`fs-cli.mjs` / `open-in-browser.mjs`（递归复制/哈希校验/配置驱动、跨平台打开）。共享工具被各专题 CLI import，只留各专题的特有编排逻辑。
- **依赖策略**：除 skill 生命周期专题的 frontmatter/YAML 解析（引入 `front-matter` 包）与交互式确认菜单外，其余均只用 Node 内建模块，保证 `node xxx.mjs` 零装包即用。
- **CLI 形态**：每个脚本以参数/子命令暴露（与 `ship.mjs` 的 `init|derive|install|check` 同调试操作风格），支持 `--dry-run` 类不落盘预演。
...
- **03-`install-verify-node`**：`install.ps1`/`verify.ps1` → `install.mjs`/`verify.mjs`，配置 `skills.config.json` 驱动，消除硬编码路径。
...
- **06-`scaffold-skills`**：setup-matt-pocock（AGENTS↔CODEBUDDY 一致性）、grill-with-memorial（NNN/脚手架/归档）、scaffold-exercises（目录树）、setup-pre-commit（lock 驱动）。
...
- 共享工具层先落地（收益高、各专题可复用），再推进各专题；`base/scripts/` 下新增工具的测试并入现有 `base/scripts/test/` 一套 `node --test`。
- 命令以 `base/scripts/` 为注册中心；各技能 SKILL.md 中调用路径统一指向共享工具与其专题 CLI，不再就地复制脚本。